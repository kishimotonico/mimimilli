...
export function buildWorkPatchTags(originalTags: string[], editedFlatTags: string[]): string[] {
  // PATCH は全置換なので、編集対象外の構造化タグを元データから必ず保持する。
  const structuredTags = originalTags.filter((tag) => parseTag(tag).kind !== "flat");
