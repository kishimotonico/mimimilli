...
describe('work api', () => {
  ...
  it('patchWork PATCHes to /api/works/:id with the given fields', async () => {
    ...
    const result = await workApi.patchWork('work-1', { title: 'new title', tags: ['tag1', 'tag2'], bookmarked: true });
    expect(mockFetch).toHaveBeenCalledWith('/api/works/work-1', expect.objectContaining({
      method: 'PATCH',
  ...
  it('getAudioUrl returns the media/audio URL', () => {
    expect(workApi.getAudioUrl('RJ001001', 'track01.mp3')).toBe('/api/media/audio/RJ001001/track01.mp3');
  });
...
describe('error handling', () => {
  ...
  it('throws an Error containing the apiErrorSchema message on failure', async () => {
    mockFetch.mockResolvedValue(makeResponse({ error: { code: 'not_found', message: '作品が見つかりません: work-1' } }, 404));
