...
import { buildWorkPatchTags, getEditableFlatTags } from "../../src/entities/work/editableTags";
...
describe("editable work tags", () => {
  it("keeps prefixed tags and replaces only flat tags", () => {
    ...
    ).toEqual(["cv/水瀬なずな", "サークル/月白製作所", "睡眠用", "ASMR"]);
  });
  ...
  it("uses parseTag boundaries for prefixed and flat values", () => {
    ...
    ).toEqual(["シリーズ/雨夜", "カテゴリ/ASMR", "custom/value", "/先頭スラッシュ", "新規"]);
  });

  it("trims flat tags and removes structured and flat duplicates", () => {
    expect(
      buildWorkPatchTags(["cv/水瀬なずな", "cv/水瀬なずな", "ASMR"], [" ASMR ", "ASMR", "", "   "]),
