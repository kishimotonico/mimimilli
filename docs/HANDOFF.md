...
**2026年5月:** mimimilli デザイン（`design_handoff_mimimilli_library/`）をベースにフロントエンドとバックエンドを全面再構築中。

- **デザイン正典:** `design_handoff_mimimilli_library/` の mimimilli モック（3ペイン Library モード・tokens.css ベースデザインシステム）
- **フロントエンド:** 既存の `src/components/*` は廃棄し、mimimilli デザインで全面置き換え予定。現時点では旧コンポーネントが残っているが、新実装と混在させない方針
...
| `design_handoff_mimimilli_library/README.md` | UIデザインシステムの正典（mimimilli）|
| `docs/DEVELOPMENT.md` | 開発者ガイド |
| `docs/web-architecture-proposal.md` | Tauri廃止・axumサーバー化の設計経緯＋新API設計 |

**廃止予定（内容は旧コンセプト基準・参照非推奨）:**

- `docs/ui-design-v1.md` — 旧グリッド/テーブル UI 設計書
- `docs/ui-brief.md` — 旧UIブリーフ
- `docs/mockup/` — 旧HTML静的モック
- `docs/wireframe-library-v4.jsx` — 旧ワイヤーフレーム JSX
