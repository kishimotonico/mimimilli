...
- ステータス: 承認
- 日付: 2026-06-11
- 関連: `docs/architecture-v2-proposal.md`（2026-07-04 削除、Git履歴参照。現行は [../ARCHITECTURE.md](../ARCHITECTURE.md)）
...
実装完了。`server/` は TypeScript（Hono）になり、旧 Rust 実装は `server-rust/` に退避した（本文中の「`server/`（Rust）」は決定当時の配置を指す）。Bun compile による単一 exe 配布とネイティブ依存回避の方針は未達で、現状は `better-sqlite3`（ネイティブモジュール）に依存している（詳細は [ARCHITECTURE.md](../ARCHITECTURE.md) 参照）。
