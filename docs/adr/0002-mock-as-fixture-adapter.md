...
- `client/` の dev サーバーはrealバックエンドサービスが指定されていないとき、workspace 依存で server の Hono アプリ（fixture アダプタ注入）をマウントする。`pnpm dev` 一発の DX とシナリオ切替（`MIMIMILLI_MOCK_SCENARIO`）を維持する
