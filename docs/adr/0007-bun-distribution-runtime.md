...
- ステータス: 承認
- 日付: 2026-07-19
- 関連: backlog TASK-70、DRAFT-27（DB分離）、DRAFT-65（zip配布）。実証スパイクは役目を終え削除済み（Git履歴 `cad3c6f` の `scripts/spike/bun-distribution/` 参照）
...
- Windows実機での本命exeの起動・DB再オープン確認は未実施。配布build実装時（DRAFT-65）に確認する。当時の手順はGit履歴 `cad3c6f` の `scripts/spike/bun-distribution/WINDOWS-SMOKE.md` にある
