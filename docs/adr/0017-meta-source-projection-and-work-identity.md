# ADR-0017: メタデータ正本・投影・作品identityを分離する
...
作品メタデータの正本ファイルは`mimimilli.json`と呼ぶ（ADR-0010）。コード識別子は`meta`系（`metaFile`・`metaPath`等）を使う。本ADRおよび関連機能では「sidecar」という語を使わない。ADR-0001・ADR-0007の「sidecar」はTauri/Node.jsのプロセスsidecarを指す別概念であり、本ADRの対象外。

## 文脈

`mimimilli.json`とcatalog SQLiteが同じ作品情報を持ち、現在の編集はcatalogを更新してから`mimimilli.json`へ書き戻している。SQLite transactionはファイル書込みを戻せないため、この順序ではcatalogだけが更新された状態を作り得る。現在のfingerprintも、正規化済みメタ、デフォルトPlaylistのmedia、作品パスを一つに混ぜており、外部編集との競合検知、投影の更新要否、移動検知を分けられない。

また、routine scanは重複したWork IDをパス順で選び、後続`mimimilli.json`のWork、Playlist、Track IDを自動再採番する。コピー先の検出順によって元作品のidentityを失わせるため、外部で管理される正本をscanが変更する方針は適さない。
...
1. 取得APIは`mimimilli.json`のexact bytesから得た`sourceRevision`を返す。
...
catalog更新が失敗しても`mimimilli.json`は正しい状態として残す。watcherまたは次回scanが再投影して収束させる。catalogを先に更新する経路、またはcatalogの値から`mimimilli.json`を組み立てる経路は廃止する。
...
| 名前                  | 入力                                                            | 用途                                   |
| --------------------- | --------------------------------------------------------------- | -------------------------------------- |
| `source_revision`     | mimimilli.jsonのexact bytes                                     | 外部編集検知と更新APIのCAS             |
...
mimimilli.jsonのWork UUIDを作品identityとする。Playlist IDとTrack IDはWork配下のローカルidentityであり、catalogのキーは`(work_id, playlist_id)`と`(work_id, playlist_id, track_id)`にする。異なるWork間でPlaylist IDまたはTrack IDが同じでも衝突ではない。これにより、複製時のidentity問題はWork IDだけに閉じる。

この方針は、ADR-0008のWork、Playlist、Track IDをライブラリ全体で一意とする決定を、Playlist/Trackについて上書きする。ADR-0008のスキャン時自動重複修復はWork IDの重複について本ADRが置き換える。同一Work内のPlaylist/Track ID重複は不正なmimimilli.jsonとしてスキーマ検証で弾き、scanとFilesで診断表示する。mimimilli.jsonを自動修復しない。
...
- mimimilli.jsonだけを複製して同じWork IDが複数箇所で見つかった場合、routine scanはmimimilli.jsonを書き換えず`identity_conflict`として診断・表示する。
...
- 複製側を「別作品として取り込む」とユーザーが明示した場合だけ、そのmimimilli.jsonのWork IDを再採番する。Playlist/Track IDはWork配下のlocal identityなので維持する。
- 音声だけを移動してmimimilli.jsonの相対パスが古くなった場合は、パスや内容から推測して追従しない。`broken reference`として表示する。

これにより、ADR-0008の「スキャン時の重複ID修復」にあるWork IDの自動再採番とパス順による所有者決定を廃止する。Work IDの重複が解消されるまで、対象のmimimilli.jsonは正本のまま保持する。

### mimimilli.json format
...
この決定の実装後、`formatVersion`がない既存の`mimimilli.json`は通常scanで補完しない。移行前にライブラリ全体を退避し、次の例のようにユーザーが明示してv1を付与する。`jq`の更新は未知フィールドを保持し、既に`formatVersion`があるファイルを変更しない。
...
移行後はcatalogだけを再構築してよい。`user.sqlite`は削除・再作成しない。catalogの再構築方法は配布時に提供する明示コマンドに従い、その後にfull scanを実行して、`identity_conflict`、不正な作品情報、broken referenceがないことを確認する。source形式の自動移行、旧形式の読取りフォールバック、互換レイヤーは追加しない。

### catalog migration 0012後の再スキャン

`server/drizzle/catalog/0012_scan_revisions.sql` で `works.source_revision` 等が追加される。既存のcatalog行は、次のスキャンが各`mimimilli.json`を再投影するまで `source_revision` が未設定のまま残る。起動時の自動フルスキャンはない。この間、作品のタイトル・タグ・ブックマークなどmimimilli.jsonへの書き戻しを伴う編集はできない。
...
スキャン完了後、各作品の `source_revision` がmimimilli.jsonのexact bytesから埋まり、編集操作が使えるようになる。

## 帰結

- mimimilli.json書込みとcatalog投影が別々に失敗し得ることを受け入れる。正本が残るため、catalogは再投影で回復できる。
