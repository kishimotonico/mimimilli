...
- 統括担当は、要件整理・分解・委任・進行管理・結果統合・差し戻し判断に専念し、原則としてコード編集・テスト実行・ブラウザ操作を行わない
- 実装担当は、実装・修正・テストコード追加を担当する。タスクが独立サブ実装へ分割できるときは、統括担当が定めた範囲・受け入れ条件の内側に限り実装担当をさらに委任して並列化してよい（委任は孫の1段まで）。委任した成果は取りまとめて統括担当へ一括報告する
- 検証担当は、テストやブラウザ検証を担当し、問題は証拠付きで統括担当へ報告する。原則として修正しない。実装担当がサブ実装を委任していた場合も、サブツリー全体の成果を最終検証する（検証所有権は検証担当に一元化する）
- Claude Codeの場合
  - 総括担当: Fable or Opus
  - 実装担当: Sonnet (もしくはCodexを利用)
  - 検証担当: Sonnet
- Codexの場合
  - 総括担当: Sol
  - 実装担当: Terra
