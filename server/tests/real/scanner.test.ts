...
test("初回スキャン: 登録・自動生成・エラー検出・duration プローブ", async () => {
  ...
  assert.ok(generated);
  assert.ok(Math.abs(generated.totalDurationSec - 5) < 0.05, `expected ~5, got ${generated.totalDurationSec}`);
...
test("UUID 重複: 後に検出された方が再採番されメタファイルへ書き戻される", async () => {
  ...
  for (const name of ["work-a", "work-b"]) {
    ...
    writeFileSync(
      ...
      JSON.stringify({ id, title: name, playlists: [{ name: "default", tracks: [{ title: "t", file: "track.wav" }] }] })
    );
  }
  const adapter = createRealAdapter({ dbPath: ":memory:" });
