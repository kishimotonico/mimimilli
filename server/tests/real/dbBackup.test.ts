...
test("createDatabaseBackupはWALを含むDBから一貫した論理スナップショットを作成する", (t) => {
  ...
  assert.equal(
    (snapshot.query("SELECT value FROM entries").get() as { value: string } | null)?.value,
    "user",
  );
  snapshot.close();
});
...
test("createDatabaseBackupは同一タイムスタンプで衝突した場合に上書きせず連番サフィックスを付ける", (t) => {
  ...
  assert.equal(
    (firstSnapshot.query("SELECT value FROM entries").get() as { value: string } | null)?.value,
    "user-v1",
  );
  assert.equal(
    (secondSnapshot.query("SELECT value FROM entries").get() as { value: string } | null)?.value,
    "user-v2",
  );
  firstSnapshot.close();
