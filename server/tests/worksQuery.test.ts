...
test("q: タグの部分一致でもヒットする", () => {
  ...
  assert.deepEqual(result.items.map((w) => w.id), ["RJ003"]);
});

test("tags: AND は全タグにマッチする作品のみ返す", () => {
  const result = applyWorksQuery(WORKS, baseQuery({ tags: ["cv/水瀬なずな", "ASMR"], tagOp: "AND" }));
  assert.deepEqual(result.items.map((w) => w.id), ["RJ001"]);
});
...
test("view: unplayed は未再生かつ status=ok のみ", () => {
  ...
  assert.deepEqual(result.items.map((w) => w.id), ["RJ002"]);
});
...
test("sort: duration-desc は再生時間の降順", () => {
  ...
  assert.deepEqual(result.items.map((w) => w.id), ["RJ003", "RJ002", "RJ001"]);
});
...
test("sort: last-played は null を末尾に並べる", () => {
  ...
  assert.deepEqual(result.items.map((w) => w.id), ["RJ001", "RJ003", "RJ002"]);
});

test("sort: id-asc はID文字列比較", () => {
  const result = applyWorksQuery(WORKS, baseQuery({ sort: "id-asc" }));
  assert.deepEqual(result.items.map((w) => w.id), ["RJ001", "RJ002", "RJ003"]);
});
