 ...
 *  tags/axis はルールに対する追加の AND 条件として適用する（ADR-0012、TASK-185）。
...
export function evalSmartFolder(
  ...
  matched = filterByTags(matched, query.tags ?? [], query.tagOp ?? "AND");
  matched = filterByAxis(matched, query.axis, query.axisValue);
