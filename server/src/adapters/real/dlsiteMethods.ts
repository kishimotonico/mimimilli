...
import { applyDlsiteStatePatch, dedupeTags, tagEquals, type DlsiteApplyBody, type DlsiteBulkMode, type DlsiteBulkProgressEvent, type DlsiteBulkResult, type DlsiteFetchResult, type DlsiteStatePatch, type NormalizedTag, type Work } from "@mimimilli/shared";
import { isDefaultTitle } from "../../core/dlsiteTitle.ts";
import { getCategoryLogger } from "../../lib/logger.ts";
import { type Db } from "./db.ts";
import { detectRjCode, fetchDlsiteCover, fetchDlsiteHtml, listDlsiteMissingFields, mergeDlsiteTags, normalizeDlsiteCoverUrl, parseDlsiteHtml } from "./dlsite.ts";
import { DEFAULT_DLSITE_CACHE_MAX_EXPANDED_BYTES, DEFAULT_DLSITE_CACHE_MAX_TRANSFER_BYTES, DlsiteCache, type DlsiteCacheOptions } from "./dlsiteCache.ts";
import { type DlsiteRequestConfig } from "./dlsiteConfig.ts";
import { DlsiteOfflineError, DlsiteScheduler, type DlsiteHttpLogContext, type DlsiteSchedulerDependencies } from "./dlsiteScheduler.ts";
...
export function createDlsiteMethods(deps: {
  ...
  const { db, repo, dlsiteCache, dlsiteCacheOptions, dlsiteRequestConfig, dlsiteScheduler, schedulerDependencies } = deps;
