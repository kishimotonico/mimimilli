...
import { type Db, type DbLocation } from "./db.ts";
...
export function createSettingsScanMethods(deps: {
  ...
  runFileScanInWorker: (database: Extract<DbLocation, { kind: "files" }>, root: string, dataRoot: string, thumbnailCacheDir: string, options: ScanOptions, testGate?: SharedArrayBuffer, testGateStage?: "before-scan" | "before-finalize", onTestGateReady?: () => void) => Promise<ScanResult>;
  scanWorkerTestGate?: SharedArrayBuffer;
  scanWorkerTestGateStage?: "before-scan" | "before-finalize";
  onScanWorkerTestGateReady?: () => void;
}) {
  const { database, repo, scanner, dataRoot, thumbnailCacheDir, runFileScanInWorker, scanWorkerTestGate, scanWorkerTestGateStage, onScanWorkerTestGateReady } = deps;
  const requireRoot = (): string => {
    const root = repo.getUserSetting(KEY_ROOT_FOLDER);
    if (!root) throw new NotConfiguredError("ルートフォルダーが設定されていません（PUT /api/settings で設定してください）");
    return root;
  };
  return {
