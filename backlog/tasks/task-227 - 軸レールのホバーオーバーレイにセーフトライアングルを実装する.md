...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 軸レールのトリガーからクイックオーバーレイへ斜め移動する際、他の軸行の上を通過してもオーバーレイが閉じない・別軸に切り替わらない
- [ ] #2 セーフトライアングル内でもポインタが約300ms静止した場合は、その軸行への切り替えが行われる
- [ ] #3 共通機構は shared/lib に置き、既存 useHoverIntent の機能（開閉遅延・pointerイベント・トリガーとパネルのタイマー共有）が失われていない
- [ ] #4 docs/design-system.md のホバーオーバーレイ規約が新機構に合わせて更新されている
- [ ] #5 三角形内判定の幾何ロジックにユニットテストがある
