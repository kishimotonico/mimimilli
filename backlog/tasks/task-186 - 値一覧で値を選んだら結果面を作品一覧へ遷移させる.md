...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 値一覧の行またはタイルを通常クリックすると、フィルタチップが追加されたうえで結果面が作品一覧へ切り替わる
- [x] #2 Ctrl+クリックまたはホバー時の追加ボタンによる AND 追加では値一覧に留まり、続けて別の値を積める
- [x] #3 遷移後に軸レールで同じ軸を選び直すと値一覧へ戻れる
- [x] #4 ブラウザの戻るで遷移前の値一覧状態へ戻れる
- [x] #5 上記の分岐が単体テストで検証されている
- [x] #6 pnpm check と pnpm test が通る
- [x] #7 置き換えは作品一覧へ進み AND 追加は現在地に留まるという規則が、値一覧だけでなくオーバーレイ類も含む全入口で一貫している（ADR-0012 §8）
<!-- AC:END -->
