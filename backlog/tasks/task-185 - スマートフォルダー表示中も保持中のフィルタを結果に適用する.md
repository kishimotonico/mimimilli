...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 スマートフォルダー軸でタグを絞り込むと、フォルダーのルールと選択タグの AND で結果が絞られる
- [ ] #2 組み込み軸のフィルタ（year）もスマートフォルダーの結果に適用される
- [ ] #3 フィルタを変更するとクエリキーが変わり、キャッシュが正しく分離される
- [ ] #4 チップ列に表示されているフィルタと実際の結果が一致している（表示だけで効かないフィルタが存在しない）
- [ ] #5 スマートフォルダー評価APIの契約変更が shared に反映され、real と fixture の両アダプタで同一に動く
