...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 並び替え選択列の開閉に collapse のアニメーションが付く
- [ ] #2 軸レールのクイックオーバーレイ・チップの兄弟値ドロップダウン・「＋絞り込み」の表示と非表示にスケール＋フェードのアニメーションが付く
- [ ] #3 transform-origin が各ポップオーバーの出現方向に合っている（右に出るものは左端、下に出るものは上端）
- [ ] #4 新規 Presence variant が prefers-reduced-motion の無効化リストに追加されている
- [ ] #5 軸行を素早く移動したときにパネルの表示が破綻しない（exit中の再オープンが正しく割り込む）ことを実機で確認済み
- [ ] #6 オーバーレイの初期位置ジャンプがアニメーションで可視化されていないことを実機で確認済み
- [ ] #7 pnpm check と変更範囲のテスト、pnpm test:smoke が通る
<!-- AC:END -->
