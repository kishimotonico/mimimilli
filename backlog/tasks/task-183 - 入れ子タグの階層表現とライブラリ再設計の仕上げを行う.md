...
- [ ] #9 作品に実際に付与されている中間タグ（例: シチュ/学園 自体がタグとして存在する）は、配下の見出しを兼ねつつ自身の件数を持つ選択可能な値として表示される
- [ ] #10 階層インデント表示は名前順ソートのときのみ適用され、件数・総時間ソートではフルパスの平坦表示にフォールバックする
- [ ] #11 作品選択時のプレビューが list/grid で共通の単一実装になっており、グリッド専用インスペクタ（WorkGridInspector・gridInspectorOpenAtom）との二重系が解消されている（TASK-180 の AC#7 から後送）
- [ ] #12 実装コメントからタスクID参照（TASK-xxx）が取り除かれ、現在の役割だけを説明する記述になっている
<!-- AC:END -->
