...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 シチュ/学園/図書室 のような3階層タグが値一覧でインデント＋葉ラベルとして表示される
- [x] #2 チップにはフルパスが表示される
- [x] #3 タグとして存在しない中間ノードが選択不可の見出しとして表示され、クリックしても絞り込みが変化しない
- [x] #4 4階層以上のタグでもレイアウトが破綻しない
- [x] #5 ContentColumn 由来の未使用 CSS クラスと未使用 atom・エクスポートがリポジトリに残っていない
- [ ] #6 ビジュアルテストに値一覧 grid・値一覧 list・チップ複合絞り込みのケースが追加され、全スナップショットが更新されて通る
- [x] #7 docs/design-system.md にチップ列・値一覧行・オーバーレイの規約が記載されている
- [x] #8 pnpm check と pnpm test が通る
- [x] #9 作品に実際に付与されている中間タグ（例: シチュ/学園 自体がタグとして存在する）は、配下の見出しを兼ねつつ自身の件数を持つ選択可能な値として表示される
- [x] #10 階層インデント表示は名前順ソートのときのみ適用され、件数・総時間ソートではフルパスの平坦表示にフォールバックする
- [x] #11 作品選択時のプレビューが list/grid で共通の単一実装になっており、グリッド専用インスペクタ（WorkGridInspector・gridInspectorOpenAtom）との二重系が解消されている（TASK-180 の AC#7 から後送）
- [x] #12 実装コメントからタスクID参照（TASK-xxx）が取り除かれ、現在の役割だけを説明する記述になっている
<!-- AC:END -->
