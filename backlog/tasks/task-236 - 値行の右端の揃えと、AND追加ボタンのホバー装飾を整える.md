...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 クイックリストの値行で、AND追加ボタンの有無にかかわらず件数の右端位置が揃う
- [ ] #2 AND追加ボタンにホバーすると行の背景より一段濃い背景が付き、ボタンの範囲が視認できる（クイックリスト・値一覧の行・タイルの3箇所）
- [ ] #3 ホバー背景の変化に短いトランジションが付いている（既存の 0.12s ease に合わせる）
- [ ] #4 IconButton の既定スタイルが変更されていない
- [ ] #5 pnpm check と変更範囲のテスト、pnpm test:smoke が通る
<!-- AC:END -->
