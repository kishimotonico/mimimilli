...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 軸レールのクイックオーバーレイの値行にホバー・フォーカスするとAND追加ボタンが現れ、押すと選択が置き換わらずAND追加される
- [x] #2 チップの兄弟値ドロップダウンでも同様にAND追加ボタンが機能する
- [x] #3 「＋絞り込み」から開く値リストにはAND追加ボタンが出ず、既存のhint表示が維持されている
- [x] #4 追加ボタンのアクセシブル名が「〇〇をAND追加」で値一覧と統一されている
- [x] #5 見出し行には追加ボタンが出ない
